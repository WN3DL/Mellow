...
import com.roxiun.mellow.util.tagignore.TagIgnoredPlayer;
import java.io.File;
...
public class TagIgnoreCommand extends CommandBase {

    private final TagIgnoreManager tagIgnoreManager;
    private final MojangApi mojangApi;
    private static final String BASE_COMMAND = "tagignore";

    public TagIgnoreCommand(
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if ("list".equalsIgnoreCase(subCommand)) {
            Map<UUID, TagIgnoredPlayer> tagIgnoreList =
                tagIgnoreManager.getTagIgnoreList();
            if (tagIgnoreList.isEmpty()) {
                ChatUtils.sendCommandMessage(
                    sender,
                    "§aThe tag ignore list is empty."
                );
                return;
            }
            ...
            List<TagIgnoredPlayer> players = new java.util.ArrayList<>(
                tagIgnoreList.values()
            );
            ...
            ChatUtils.sendCommandMessage(
                sender,
                "§aPlayers on your tag ignore list (Page " +
                page +
                "/" +
                totalPages +
                "):"
            ...
            for (int i = startIndex; i < endIndex; i++) {
                TagIgnoredPlayer player = players.get(i);
            ...
            AsyncExecutor.getInstance().command(() -> {
                MainThreadDispatcher.run(() -> {
                    ChatUtils.sendCommandMessage(
                        sender,
                        "§eImporting tag ignore list with file: " +
                            syncFile.getAbsolutePath()
                    );
                });

                try {
                    int newEntries = tagIgnoreManager.syncWithExternalFile(syncFile);
                    String message;
                    if (newEntries > 0) {
                        message =
        ...
        AsyncExecutor.getInstance().command(() -> {
            ...
            if ("add".equalsIgnoreCase(subCommand)) {
                ...
                boolean playerAdded = tagIgnoreManager.addPlayer(
                    uuid,
                    playerName,
                    reason
                );
                if (playerAdded) {
                    MainThreadDispatcher.run(() ->
                        ChatUtils.sendCommandMessage(
                            sender,
                            "§aAdded " + playerName + " to the tag ignore list."
                        )
                    );
                } else {
                    MainThreadDispatcher.run(() ->
                        ChatUtils.sendCommandMessage(
                            ...
                            "§c" +
                                playerName +
                                " is already on the tag ignore list for reason: " +
                                tagIgnoreManager
                                    .getTagIgnoredPlayer(uuid)
                                    .getReason()
                        )
                    );
                }
            } else if ("remove".equalsIgnoreCase(subCommand)) {
                tagIgnoreManager.removePlayer(uuid);
                MainThreadDispatcher.run(() ->
                    ChatUtils.sendCommandMessage(
                        sender,
                        "§aRemoved " + playerName + " from the tag ignore list."
                    )
                );
            } else {
                MainThreadDispatcher.run(() ->
